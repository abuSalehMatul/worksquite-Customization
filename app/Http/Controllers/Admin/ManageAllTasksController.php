    ...
    public function data(Request $request, $startDate = null, $endDate = null, $hideCompleted = null, $projectId = null) {
        ...
        $tasks = Task::leftJoin('projects', 'projects.id', '=', 'tasks.project_id')
           ...
           // ->join('users', 'users.id', '=', 'tasks.user_id')
            ...
            ->select('tasks.id','tasks.user_id', 'projects.project_name', 'tasks.heading', 'client.name as clientName',  'creator_user.name as created_by', 'creator_user.image as created_image', 'tasks.due_date', 'taskboard_columns.column_name', 'taskboard_columns.label_color', 'tasks.project_id');
        ...
        return DataTables::of($tasks)
            ...
            ->addColumn('name', function ($row) {
                ...
                $assignee_user=User::find($user_id[0]);
                return ($row->id) ? '<img src="' . asset('user-uploads/avatar/' . $assignee_user->image) . '"
                                                            alt="user" class="img-circle" width="30"> ' . ucwords($assignee_user->name) : '<img src="' . asset('default-profile-2.png') . '"
                                                            alt="user" class="img-circle" width="30"> ' . ucwords($assignee_user->name);
            })
